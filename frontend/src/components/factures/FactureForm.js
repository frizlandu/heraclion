...
const FactureForm = ({ facture, mode, onSubmit, onCancel, onClose, onSave }) => {
  ...
  useEffect(() => {
    console.log('FactureForm - Initialisation:', { facture, mode, lignes: facture?.lignes });
    
    if (facture && mode === 'edit') {
      console.log('FactureForm - Données facture complètes:', facture);
      console.log('FactureForm - Lignes de la facture:', facture.lignes?.map(ligne => ({
        id: ligne.id,
        item: ligne.item,
        unite: ligne.unite,
      ...
      // Préparer les lignes en préservant tous les champs spécifiques
      let lignesInitialisees = [];
      
      if (facture.lignes && facture.lignes.length > 0) {
        // Préserver toutes les lignes existantes avec tous leurs champs
        lignesInitialisees = facture.lignes.map(ligne => ({
          // Champs communs
          id: ligne.id || Date.now() + Math.random(),
          description: ligne.description || '',
          prix_unitaire: parseFloat(ligne.prix_unitaire) || 0,
          taux_tva: ligne.taux_tva != null ? parseFloat(ligne.taux_tva) : 0,
          ...
          // Champs spécifiques transport (initialiser avec valeurs par défaut si undefined)
          item: ligne.item != null ? ligne.item : '',
          date_transport: ligne.date_transport != null ? ligne.date_transport : '',
          plaque_immat: ligne.plaque_immat != null ? ligne.plaque_immat : '',
          ticket: ligne.ticket != null ? ligne.ticket : '',
          tonnes: ligne.tonnes != null ? parseFloat(ligne.tonnes) : 0,
          total_poids: ligne.total_poids != null ? parseFloat(ligne.total_poids) : 0,
          frais_administratif: ligne.frais_administratif != null ? parseFloat(ligne.frais_administratif) : 0,
          
          // Champs spécifiques non-transport (initialiser avec valeurs par défaut si undefined)
          unite: ligne.unite != null ? ligne.unite : '',
          quantite: ligne.quantite != null ? parseFloat(ligne.quantite) : 1
        }));
      } else {
        // Créer une ligne par défaut si aucune ligne existante
        lignesInitialisees = [
          {
            id: Date.now(),
            description: '',
  ...
  useEffect(() => {
    setFormData(prev => {
      const newLignes = prev.lignes.map(ligne => {
        ...
        // Préserver TOUS les champs de la ligne originale
        ...
        // Calcul automatique du total/poids pour les factures transport (reste en tonnes)
        if (formData.categorie_facture === 'transport') {
          const tonnes = parseFloat(ligne.tonnes) || 0;
          // total_poids = tonnes (pas de conversion en kg)
          ligneModifiee.total_poids = tonnes;
        }
  ...
  const handleLigneChange = (index, field, value) => {
    setFormData(prev => {
      ...
      if (['quantite', 'prix_unitaire', 'taux_tva', 'frais_administratif', 'tonnes'].includes(field)) {
        const prixUnitaire = parseFloat(ligne.prix_unitaire) || 0;
        const tauxTva = parseFloat(ligne.taux_tva) ?? 0;
        
        let quantiteCalcul;
        let fraisAdministratif = 0;
        
        if (prev.categorie_facture === 'transport') {
          // Pour Transport: utiliser total_poids (en tonnes) comme quantité
          quantiteCalcul = parseFloat(ligne.total_poids) || 0;
          fraisAdministratif = parseFloat(ligne.frais_administratif) || 0;
        } else {
          // Pour Non-Transport: utiliser quantité normale
          quantiteCalcul = parseFloat(ligne.quantite) || 0;
          fraisAdministratif = 0; // Pas de frais admin pour Non-Transport
        }
        
        const montants = calculerLigne(quantiteCalcul, prixUnitaire, tauxTva, fraisAdministratif);
      ...
      // Total/Poids reste en tonnes (pas de conversion en kg)
      if (field === 'tonnes') {
        const tonnes = parseFloat(value) || 0;
        ligne.total_poids = tonnes; // Garder en tonnes
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Nettoyage des données pour éviter les valeurs NaN ou undefined
      const factureData = {
        ...formData,
        ...
        // S'assurer que categorie_facture est bien incluse
        categorie_facture: formData.categorie_facture,
        // Mapper description vers notes (colonne réelle en BDD)
        notes: formData.description,
        // Mapping des montants pour correspondre au backend
        montant_ht: parseFloat(totaux.total_ht.toFixed(2)),
        montant_ttc: parseFloat(totaux.total_ttc.toFixed(2)),
        taux_tva: 16, // TVA fixe à 16%
        // Conserver aussi les totaux pour compatibilité
        ...
        // Nettoyer les lignes selon le type de facture
        lignes: formData.lignes.map(ligne => {
          ...
          if (formData.categorie_facture === 'transport') {
            // Champs spécifiques Transport
            ligneCleaned.date_transport = ligne.date_transport || '';
            ligneCleaned.plaque_immat = ligne.plaque_immat || '';
            ligneCleaned.ticket = ligne.ticket || '';
            ligneCleaned.tonnes = parseFloat(ligne.tonnes) || 0;
            ligneCleaned.total_poids = parseFloat(ligne.total_poids) || 0;
            ligneCleaned.frais_administratif = parseFloat(ligne.frais_administratif) || 0;
            ligneCleaned.quantite = parseFloat(ligne.tonnes) || 0; // Pour transport, quantité = tonnes
          } else {
            // Champs spécifiques Non-Transport
            ligneCleaned.unite = ligne.unite || '';
            ligneCleaned.quantite = parseFloat(ligne.quantite) || 1;
            // Pas de frais administratifs pour Non-Transport
            ligneCleaned.frais_administratif = 0;
          }
      ...
      if (!factureData.entreprise_id || !factureData.client_id) {
        throw new Error('Entreprise et client sont requis');
      }
      ...
      // Vérifier que toutes les lignes ont des montants valides
      const lignesInvalides = factureData.lignes.filter(ligne => 
        !ligne.description || 
        ligne.prix_unitaire <= 0 || 
        (formData.categorie_facture !== 'transport' && ligne.quantite <= 0)
      );

      if (lignesInvalides.length > 0) {
        throw new Error('Toutes les lignes doivent avoir une description, un prix unitaire et une quantité valides');
      }

      console.log('Données à sauvegarder:', factureData);
      console.log('DEBUG - Catégorie facture avant envoi:', {
        categorie_facture: factureData.categorie_facture,
        formData_categorie: formData.categorie_facture,
        type_document: factureData.type_document
      });
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border border-gray-200 rounded-md">
                <thead className="bg-gray-50">
                  <tr>
                    {formData.categorie_facture === 'transport' ? (
                      // Colonnes pour factures TRANSPORT
                      <>
                        ...
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tonne</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Total/Poids</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Prix unitaire</th>
                      ...
                      // Colonnes pour factures NON-TRANSPORT (identiques à FactureView.js)
                      <>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Item</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Désignation</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Unité</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Quantité</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Prix unitaire</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Total (HT)</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">TVA%</th>
                        <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Total (TVA)</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {formData.lignes.map((ligne, index) => (
                    <tr key={ligne.id || index}>
                      {formData.categorie_facture === 'transport' ? (
                        // Ligne TRANSPORT - Toutes les colonnes
                        <>
                          {/* Item */}
                          ...
                          <td className="px-2 py-2">
                            <input
                              ...
                              readOnly
                              placeholder="Auto"
                              title="Calculé automatiquement en tonnes (= Tonnes saisies)"
                            />
                          </td>
                        ...
                        // Ligne NON-TRANSPORT (identique à FactureView.js)
                        <>
                          {/* Item */}
                          <td className="px-2 py-2">
                            <input
                          ...
                          <td className="px-2 py-2">
                            <input
                              ...
                              value={ligne.description}
                              onChange={(e) => handleLigneChange(index, 'description', e.target.value)}
                              className="w-40 p-1 border border-gray-300 rounded text-sm"
